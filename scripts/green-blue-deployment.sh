...
function create_deployment() {  
  ...
  if [  $1 == $ASC_DEPLOYMENT_COLOR_1 ] && [ $2 == 1 ]; then
    ...
    INSTANCE_STATUS=${INSTANCE_STATUS:az spring-cloud app deployment list --app $ASC_APP_NAME --resource-group $ASC_RESOURCE_GROUP_NAME --service $ASC_SERVICE_NAME --query "[?name=='${NAME}'].properties.provisioningState | [0]"}
